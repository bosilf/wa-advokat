...
export default function EmployeeCard({ employee }: EmployeeCardProps) {
  ...
  return (
    <li>
      <article ref={containerRef} className="aspect-2/3 rounded-lg w-full bg-white overflow-hidden group block flex flex-col justify-between transition-all duration-400  hover:drop-shadow-lg">
        {employee.image && (
          <Link className="h-full flex-1" href={`/medarbetare/${employeeSlug}`}>
            <div className="overflow-hidden h-full relative">
              <h3 className="z-10 absolute h-full inset-0 bg-accent/70 backdrop-blur-lg opacity-0 group transition-all duration-300 hover:opacity-100 flex flex-col items-center justify-center gap-1 select-none flex flex-col justify-center text-white font-body">Läs mer om <span className="duration-300 flex w-fit gap-0 group-hover:gap-sm">{employee.firstName} <Icon name="arrow" size={15} className="text-white" /></span></h3>
              <Image
                ref={imageRef}
          ...
          <Link href={`/medarbetare/${employeeSlug}`}>
            ...
            <div className="mt-2 h-min flex gap-1 text-body divide-x-1 font-caption">
              {employee.roles?.map((role, index: number) => {
                ...
                return (
                  <span className="flex justify-center flex-row pr-1" key={index}>
                    <Link  
