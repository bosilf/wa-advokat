...
export default function HeroCourse({
  ...
  return (
    <section className="bg-canvas z-10 flex h-svh min-h-screen flex-col overflow-hidden">
      ...
      <div
        className={`
          bg-footer md:bg-footer shrink-0 py-lg pr-lg
          shadow-[inset_0px_20px_15px_-11px_rgba(0,0,0,0.10)]
          transition-all transform-all duration-500 ease-out
          ${hasScrolled ? "rounded-br-xl" : "rounded-b-none"}
        `}
      >
        <h1 className="flex flex-col gap-sm p-lg md:pl-0 text-white">
          ...
          {title && (
            <span className="font-display md:text-white  md:py-md md:pl-xl md:pr-xl w-fit lg:max-w-[80%] px-lg">
              {title}
            </span>
          )}
        </h1>
