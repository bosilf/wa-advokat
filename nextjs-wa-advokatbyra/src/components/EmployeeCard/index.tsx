...
export default function EmployeeCard({
  ...
  return (
    <li>
      <article
        ref={containerRef}
        className="aspect-2/3 rounded-lg w-full bg-white overflow-hidden group flex flex-col justify-between transition-all duration-400 hover:drop-shadow-lg"
      >
        {employee.image && (
          <Link
            className="h-full flex-1"
            href={`/medarbetare/${employeeSlug}`}
          >
        ...
        <div className="flex flex-col p-6 bg-white">
          ...
          <div className="mt-2 h-min flex gap-1 text-body divide-x-1 font-caption">
            {employee.roles?.map((role, index) => {
              ...
              return (
                <span
                  className="flex justify-center flex-row pr-1 pl-1 first:pl-0"
                  key={`${roleSlug}-${index}`}
                >
                  <Link
